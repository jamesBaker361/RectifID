...
class PersonRFlow:
    def __init__(self, model, seed, device,torch_dtype,size):
        ...
        self.model = convert('glintr100.onnx').eval().to(self.device)
    ...
    def prepare(self, ref:Union[str,Image.Image],ref_text:str=None):
        if type(ref)==str:
        ...
        with torch.no_grad():
            ...
            ref_image_cropped = kornia.geometry.transform.warp_affine(
                TF.to_tensor(ref_image).unsqueeze(0).to(self.device), torch.tensor(M).float().unsqueeze(0).to(self.device), (112, 112)
            ) * 2 - 1
    ...
    def generate(self, prompt, seed, out, num_iterations=50, num_steps=4, verbose=True, guidance=1.)->Image.Image:
        ...
        for iteration in iterations:
            ...
            image_cropped = kornia.geometry.transform.warp_affine(
                image.float().unsqueeze(0), torch.tensor(M).float().unsqueeze(0).to(self.device), (112, 112)
            ) * 2 - 1
